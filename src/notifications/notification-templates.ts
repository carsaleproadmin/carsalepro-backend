...
const CATALOG: Record<NotificationType, Record<NotificationLocale, TemplateFn>> = {
  ...
  'listing.hidden': {
    de: (p) => ({
      subject: `Anzeige ausgeblendet`,
      body:
        `Die Plattform hat Ihre Anzeige für ${str(p, 'make')} ${str(p, 'model')} aus dem Showroom entfernt.\n\n` +
        `Grund: ${str(p, 'reason')}\n\n` +
        `Sie können die Anzeige bearbeiten oder löschen. Wieder veröffentlichen kann sie nur die Plattform. ` +
        `Bei Fragen wenden Sie sich an den Support.`,
      short: `Anzeige für ${str(p, 'make')} ${str(p, 'model')} ausgeblendet.`,
    }),
    en: (p) => ({
      subject: `Listing hidden`,
      body:
        `The platform removed your listing for ${str(p, 'make')} ${str(p, 'model')} from the showroom.\n\n` +
        `Reason: ${str(p, 'reason')}\n\n` +
        `You can edit or delete the listing. Only the platform can publish it again. ` +
        `If you have questions, contact support.`,
      short: `Listing for ${str(p, 'make')} ${str(p, 'model')} hidden.`,
    }),
    ru: (p) => ({
      subject: `Объявление скрыто`,
      body:
        `Платформа убрала ваше объявление о ${str(p, 'make')} ${str(p, 'model')} из шоурума.\n\n` +
        `Причина: ${str(p, 'reason')}\n\n` +
        `Вы можете изменить или удалить объявление. Снова опубликовать его может только платформа. ` +
        `Если есть вопросы, напишите в поддержку.`,
      short: `Объявление о ${str(p, 'make')} ${str(p, 'model')} скрыто.`,
    }),
  },
  'listing.unhidden': {
    de: (p) => ({
      subject: `Anzeige wieder sichtbar`,
      body: `Ihre Anzeige für ${str(p, 'make')} ${str(p, 'model')} ist wieder im Showroom sichtbar.`,
      short: `Anzeige für ${str(p, 'make')} ${str(p, 'model')} wieder sichtbar.`,
    }),
    en: (p) => ({
      subject: `Listing restored`,
      body: `Your listing for ${str(p, 'make')} ${str(p, 'model')} is visible in the showroom again.`,
      short: `Listing for ${str(p, 'make')} ${str(p, 'model')} restored.`,
    }),
    ru: (p) => ({
      subject: `Объявление снова видно`,
      body: `Ваше объявление о ${str(p, 'make')} ${str(p, 'model')} снова видно в шоуруме.`,
