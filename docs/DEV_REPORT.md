...
Bundle ID is checked against `IAP_BUNDLE_ID` (default `net.carsalepro.app`; the retired `com.carsalepro.app` and `us.designkey.carsalepro` are ignored if the environment still holds one) and the request is rejected on mismatch.
