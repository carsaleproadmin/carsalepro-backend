...
- **The report completeness gate is a setting, not a constant.** `minReportQualityScore` (PlatformSetting, seeded 90) is asserted in `attachReportByCode` and `ReportsService.create`; `0` disables it and is the deliberate emergency lever; above 100 is refused, since it would silently refuse every report. A **missing** score is its own code (`report_quality_unknown`) — that means an old app build, not poor work.
