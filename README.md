...
| Testing | Jest unit + Supertest e2e (**519 e2e across 26 suites**, 738 unit across 38) |
...
- **Mobile PRO is a single 7.00 EUR purchase, not a subscription.** The product is `carsalepro_pro_lifetime` (Apple Non-Consumable, Google managed in-app product) and it grants a permanent `DeviceQuota.isPro` with no expiry. `GOOGLE_PLAY_SUBSCRIPTION_IDS` must stay empty, or the id is sent to the subscriptions endpoint and every purchase 404s. Website Stripe subscriptions are a different product and are unaffected.
- **The paint walk is 17 guided stations.** The four door sills were added on 2026-08-19 in the client's dictated order, so the left flank is deliberately not a mirror of the right, and two stations per side share one `partId` (`rocker_panel_left` / `_right`) — which is why a station's own label now wins over its part's label when the part is shared.
- **`GET /legal/:doc` falls back to English, not German.** It served German to any language it could not match, which was 31 of the app's 35 locales, and the company details in `legal.content.ts` are now filled rather than bracketed placeholders. A spec asserts no `[` survives in any rendered section in any supported language.
- **Orders authorize first and capture at acceptance** (the ride-hailing model). Creating an order places a **hold** (`capture_method: 'manual'`); `payment_intent.amount_capturable_updated` starts a search window (`orderSearchWindowMinutes`, 6 h) and dispatches; the funds are taken only when an inspector accepts, so an order is never `ASSIGNED` with uncaptured money. Nobody accepts → a cron releases the hold and cancels. Cancelling before acceptance therefore answers `refundCents: 0` + `refundMode: "authorization_released"` and writes **no** `Refund` row — the money never left the customer. Money state lives on `Payment` (`pending → authorized → succeeded`, or `cancelled`), not in `OrderStatus`: `PAID` has always meant "the customer's money is committed", and held-versus-taken is a payment fact. `GET /orders/:id` carries optional `payment` / `search` / `reportRequirement` blocks so the website can render the difference without depending on deploy order. **Stripe must be subscribed to `payment_intent.amount_capturable_updated`, `.canceled` and `.payment_failed`.**
- **An order can only be closed with a complete report.** The gate runs on both `POST /orders/:id/report` and `POST /reports` with an `orderId`, and since 2026-08-13 it checks which elements the report's `reportData` payload contains rather than comparing a score to a threshold. `minReportQualityScore` (PlatformSetting) kept its name but is now only a lever: `0` turns the gate off from the admin panel, anything above it turns the gate on. It requires every exterior angle photographed, each of the 17 paint panels with a reading **and** a photo, both calibration shots, and all four wheels with tread, DOT and size. A report with no structured payload is `report_quality_unknown` (update the app); one with gaps is `report_incomplete`, carrying the structured `missing` list of what to go back for. **Both required lists carry a legacy amnesty**, so growing a list never strands the apps already in the field: an older 8-angle report is judged against the walk-around it was filed under, and a payload that names no door sill is judged against the 13 paint stations its build shipped with. Because of that, and because every array cap is a raise, **the backend deploys before the app** — an older app against the new API is fine, a newer app against the old one is the failure mode. `GET /orders/:id` exposes the required counts in `reportRequirement`.
